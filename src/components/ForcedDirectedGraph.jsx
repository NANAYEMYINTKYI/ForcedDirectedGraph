...
const imageslist = import.meta.glob("../asset/image/*.{png,jpg,jpeg,svg}", {
  eager: true,
});
...
const ForceDirectedGraph = ({
  ...
  const dragstarted = useCallback(function (event, d) {
    if (!event.active)
      simulationRef.current.alphaTarget(0.01).alpha(1.01).restart();
    d.fx = d.x;
    d.fy = d.y;
  ...
  const dragended = useCallback(function (event, d) {
    if (!event.active) simulationRef.current.alphaTarget(0.5).alpha(1.01);
  ...
  useEffect(() => {
    ...
    function zoomToNode(d) {
      ...
      const scale = Math.min(Math.max(targetRadius / avgDistance, 0.5), 3);
      ...
      setTimeout(() => {
        simulationRef.current.force("tempAttract", null).alphaTarget(0);
      }, 1500);
    }
    ...
    node
      ...
      .on("mouseenter", (event, d) => {
        ...
        if (d.group === 9) {
          ...
          tooltipRef.current.style("opacity", 1).html(`<strong>${
            d.id
          }</strong><br/> 
    ...
    if (selectnode) {
      setTimeout(() => {
        const select = nodes.find(
          (n) =>
            n.id.toLowerCase().replace(/\s+/g, "_") ===
    ...
    node
      ...
      .each(function (d) {
        const current = d3.select(this);
        if (d.file) {
          current.attr("y", d.size + 5).style("font-size", "12px");
        } else {
          current.attr("y", -50).style("font-size", (d) => d.size / 4);
        }
      })
      .attr("x", (d) => -d.size * 1)
